	...
	public boolean isFulfilledBy(IJavaType type, SubtypeChecker subtypeChecker, IJavaDebugTarget target) {
		for (Method method: ExpressionGenerator.getMethods(type)) {
			if ((methodName == null || method.name().equals(methodName)) && method.argumentTypeNames().size() == argConstraints.size()
					&& methodConstraint.isFulfilledBy(EclipseUtils.getFullyQualifiedTypeIfExists(method.returnTypeName(), target), subtypeChecker, target)) {  // We use the ifExists version because it might be void.
				int i = 0;
